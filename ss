...
local getgenv, ipairs, typeof, pairs, Vector3_new, pcall = getgenv, ipairs, typeof, pairs, Vector3.new, pcall
	...
	if (Aiming.IsIgnored(Player) == false and Character) then
		...
		if (TargetPartTemp and Aiming.CheckHealth(Player)) then
			...
			if (circle.Radius > Magnitude and Magnitude < ShortestDistance) then
				-- // Check if Visible
				if (Aiming.VisibleCheck and not Aiming.IsPartVisible(TargetPartTemp, Character)) then end
...
for i,v in pairs(game.Players:GetChildren()) do
	if v.Name ~= game.Players.LocalPlayer.Name then
		...
		hrp.AssemblyLinearVelocity = Vector3_new(hrp.Velocity.X, 0, hrp.Velocity.Z)
	end
end
end)
end)
